...
#include <stdlib.h>
#include <unistd.h>
#include <string.h>  // Include for memcpy and strcmp
...
static int read_grey_8bit_png(FILE *fl, void **image, int *width, int *height) {
    ...
    // Convert PNG to grayscale if it is not already
    if (color_type == PNG_COLOR_TYPE_RGB || color_type == PNG_COLOR_TYPE_RGBA) {
        png_set_rgb_to_gray(png, 1, -1, -1);
    }

    // Update the info struct with the result of the transformations
